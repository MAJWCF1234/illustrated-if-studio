#!/usr/bin/env node
/** Find a Python 3 and run scripts/python-rollback-smoke.py */
...
const script = path.join(studio, "scripts", "python-rollback-smoke.py");
...
function tryRun(cmd) {
  ...
  const runArgs = cmd === "py" ? ["-3", script] : [script];
...
console.error("No Python 3 on PATH — skipped python rollback smoke.");
process.exit(0);
