...
class Runtime {
 ...
 private:
  ...
  // Scenes auto-advanced through in the current skip run, so a story that loops
  // back (A -> B -> A) stops instead of skipping forever.
  std::unordered_set<std::string> skipChain_;
...
export function exportCpp({ studioRoot, projectDir, outRoot }) {
  ...
  const author = typeof project.author === "string" ? safeLabel(project.author, "") : "";
  fs.writeFileSync(path.join(staging, "README.md"), README(title, author));
  fs.writeFileSync(
